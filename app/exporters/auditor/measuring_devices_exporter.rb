        ...
        wb.add_worksheet(:name => subject_type.model_name.human(:count => 2), :page_margins => margins, :page_setup => setup) do |sheet|
          headers = [
            ...
            {:titles => ["", "#{subject_type.model_name.human if subject_type}",
                "Количество установленных приборов учета по состоянию на #{period.start_date_localized} г.",
                Array.new(kinds_count).map!{""},
                "Необходимое количество приборов учета до 100% оснащения на #{period.start_date_localized} г.",
                Array.new(kinds_count).map!{""},
                "Фактическое количество установленных приборов учета по состоянию на #{period.date_localized} г.",
                Array.new(kinds_count).map!{""},
                "Процент оснащённости",
              ].flatten,
