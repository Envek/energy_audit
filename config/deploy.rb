# call with `cap -S domain="<domain>" deploy` to deploy to another server.
...
set :application, "energy_audit"
set :rails_env, "production"
set :domain, exists?(:domain) && !domain.empty? ? domain : "andrey@aries.amurobl.ru" # Это необходимо для деплоя через ssh.
...
set :scm, :git
set :repository,  "git@github.com:Envek/energy_audit.git" # Путь до вашего репозитария. Кстати, забор кода с него происходит уже не от вас, а от сервера, поэтому стоит создать пару rsa ключей на сервере и добавить их в deployment keys в настройках репозитария.
